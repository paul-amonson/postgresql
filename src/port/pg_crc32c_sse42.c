...
pg_comp_crc32c_sse42(pg_crc32c crc, const void *data, size_t length)
{
	...
	if (length < 288)
	{
		return pg_comp_crc32c_sse42_old(crc, data, length);
	}
	...
	if ((uint64)input % AVX_ALIGN != 0)
	{
		...
		crc = pg_comp_crc32c_sse42_old(crc, data, slop);
		input += slop;
		length -= slop;
	}
	...
	if (length > 0)
	{
		...
		crc = pg_comp_crc32c_sse42_old(crc, input, length);
	}
	return crc;
}
...
// 	pg_crc32c newValue;
// 	pg_crc32c oldValue;
// 	newValue = pg_comp_crc32c_sse42_new(saved, data, length);
// 	dump("New CRC", (size_t)newValue);
// 	oldValue = pg_comp_crc32c_sse42_old(saved, data, length);
// 	dump("Old CRC", (size_t)oldValue);
// 	return newValue;
// }
